...
import { Brain, Send, Volume2, VolumeX, MessageSquare } from 'lucide-react';
...
export default function VoiceAIPage({ setCurrentTab }) {
  ...
  const content = (
    <div className="animate-fade-in flex flex-col h-[calc(100vh-120px)] lg:h-[calc(100vh-60px)] font-sans max-w-5xl mx-auto w-full">
      ...
      {/* Main chat window */}
      <div className="flex-1 overflow-y-auto space-y-5 pr-2 p-4 lg:p-6 bg-[#090909] border border-white/[0.04] rounded-3xl layered-shadow-lg mb-4 lg:mb-6 flex flex-col">
        {messages.map((msg, idx) => (
          ...
          >
            {msg.role === 'ai' && (
              <div className="flex items-center gap-1.5 shrink-0 px-2 py-0.5 rounded-md bg-[#E5B842]/10 border border-[#E5B842]/20">
      ...
      <div className="space-y-6 shrink-0 mt-auto">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {sampleQuestions.map((qItem, idx) => (
            ...
            >
              <MessageSquare className="w-3.5 h-3.5 text-[#E5B842]" /> {qItem.q}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 relative flex items-center">
            <input 
              ...
              className="w-full bg-[#0B0B0B] border border-[#E5B842]/20 hover:border-[#E5B842]/40 focus:border-[#E5B842]/60 shadow-inner rounded-2xl pl-5 pr-14 py-4 text-base text-white focus:outline-hidden transition-all duration-300"
            />
            <button 
              onClick={() => handleSend(inputText)}
              className="absolute right-3 p-2 bg-[#E5B842]/10 hover:bg-[#E5B842]/20 text-[#E5B842] rounded-xl transition-colors cursor-pointer"
            >
