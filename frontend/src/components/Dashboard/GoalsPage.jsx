...
export default function GoalsPage() {
  ...
  const [manualMilestones, setManualMilestones] = useState([{ week: 1, text: '', effort: 'medium' }]);
  ...
  const handleAddGoal = async (e) => {
    ...
    try {
      ...
      setAutoPlan(false);
      setUserPreferences('');
      setManualMilestones([{ week: 1, text: '', effort: 'medium' }]);
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans relative">
      ...
      <form onSubmit={handleAddGoal} className="p-6 bg-[#090909] border border-white/[0.04] rounded-2xl space-y-4 layered-shadow-lg">
        <h3 className="text-[10px] font-bold text-[#E5B842] tracking-widest uppercase font-display mb-4">Launch Strategic Target</h3>
        ...
        <div className="space-y-4 pt-4 border-t border-white/5">
          <div className="animate-fade-in-up space-y-3">
            <label className="text-[9px] font-tech font-bold uppercase tracking-wider text-[#E5B842] block mb-2 flex items-center gap-1.5">
              ...
              <div className="space-y-3">
                {manualMilestones.map((m, index) => (
                  <div key={index} className="flex gap-3 items-start bg-white/[0.02] border border-white/5 p-3 rounded-xl relative group">
                    <div className="pt-2 text-[10px] font-bold text-white/40 uppercase w-12 shrink-0">
                      Wk {m.week}
                    </div>
                    ...
                    <textarea 
                      ...
                      className="flex-1 bg-[#050505] border border-white/10 focus:border-[#E5B842]/40 rounded-lg px-3 py-2 text-xs text-white placeholder-white/20 focus:outline-hidden resize-none h-10"
                    />
                    ...
                    {manualMilestones.length > 1 && (
                      <button 
                        ...
                        className="text-white/20 hover:text-red-400 p-2 opacity-0 group-hover:opacity-100 transition-opacity absolute -right-2 -top-2 bg-[#090909] rounded-full border border-white/10 cursor-pointer"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    )}
              ...
              <button 
                type="button" 
                onClick={addWeek}
                className="text-[10px] text-[#E5B842]/70 hover:text-[#E5B842] uppercase font-bold tracking-wider flex items-center gap-1 transition-colors cursor-pointer"
              >
                      ...
                      goal.milestones.map((milestone) => (
                        ...
                        >
                          ...
                          <span className={`text-[13px] leading-relaxed ${milestone.done ? 'line-through text-white/35' : 'text-white/80'}`}>
                            Week {milestone.week}: {milestone.milestone} <span className="text-[9px] text-[#E5B842]/80 uppercase tracking-widest ml-1.5 font-bold">({milestone.effort} effort)</span>
                          </span>
                        </div>
                      ))
                    ) : (
                ...
                {goal.milestones && goal.milestones.length > 0 && (
                  <div className="p-4 bg-white/[0.01] border border-[#E5B842]/20 rounded-2xl relative overflow-hidden card-shine-sweep mt-4">
                    ...
                    <p className="text-sm text-white/80 font-normal leading-relaxed mt-1.5">
                      "Gemini has successfully analyzed your goal and created a simple, step-by-step {goal.milestones.length}-week plan for you to follow."
                    </p>
                  </div>
                )}
